...
import { ItemsPageState, ExpandedItemPageState } from '../../interfaces/ItemsPageState';
...
const ExpandedItemView: React.FC = () => {
  ...
  const [item, setItem] = useState<Motif | Structure | undefined>(initialItem);
  ...
  const restoredMotifPageState = (location.state?.motifPageState as ItemsPageState) || {};
  ...
  const isMotif = (item: Motif | Structure): item is Motif => item?.type === 'motif';
  ...
  return (
    <div>
      ...
      {!isMotif(item) && <span>(Structure ID: {item.id || 'Unnamed Structure'})</span>}
      <SvgViewer
        svgXML={isMotif(item) ? item.svg : item.svgContent?.[currentSvgIndex] || ''}
      ...
      {!isMotif(item) && item.svgContent && (
        <div className="svg-navigation">
          <button
            ...
            className={`nav-button ${isSingleSvg ? 'disabled' : ''}`}
          >
            &lt; Prev
          </button>
          <span className="svg-index">
            View {currentSvgIndex + 1} / {isMotif(item) ? 1 : item.svgContent?.length || 1}
          </span>
          <button
      ...
      <p>
        ...
        {isMotif(item) ? (
          ...
          <>
            ...
            <br />
            Unique Minimal Undesignable Motifs: {item.motifsID.length}{' '}
          </>
        )}
      </p>
      {/* Dot-Bracket Structure */}
      <CopyableTextBlock
        text={isMotif(item) ? item.dotBracket?.[0] || '' : item.dotBracket || ''}
      ...
      <ItemsGridView
        ...
        itemIDs={
          isMotif(item) ? processStructureIDs(item.structures_id) : processMotifIDs(item.motifsID)
        }
