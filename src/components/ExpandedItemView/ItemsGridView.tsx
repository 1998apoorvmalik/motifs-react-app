...
const ItemsGridView: React.FC<GridProps> = ({
  ...
  useEffect(() => {
    const fetchStructures = async () => {
      ...
      try {
        ...
        if (itemType === 'motif') {
          data = await Promise.all(paginatedItemIDs.map(id => motifService.getMotif(id)));
        } else {
          data = await Promise.all(
            paginatedItemIDs.map(id => structureService.getStructure(id, itemIDs[id]))
          );
        }
  ...
  if (loading) {
    return (
      <div className="centered-container" style={{ minHeight: '50vh', flexDirection: 'column' }}>
  ...
  return (
    <div className="container">
      <div className="header-bar" style={{ paddingTop: '4px', paddingBottom: '4px' }}>
        <Pagination
